...
void  OSIntExit(void)
{
	...
	if (OSRunning == OS_TRUE) {
		...
		if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
			if (OSLockNesting == 0u && OSTCBCur->compTime) {                     /* ... and not locked.                      */
				OS_SchedNew();
				OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];
				if (OSPrioHighRdy != OSPrioCur) {          /* No Ctx Sw if current task is highest rdy */

				//发生抢占时输出
				APP_TRACE("%d\tPreempt\t\t%d\t%d \n", OSTimeGet(), OSPrioCur, OSPrioHighRdy);
...
void  OSStart(void)
{
	if (OSRunning == OS_FALSE) {
		...
		//进程开始
		APP_TRACE("%d\tPreempt\t\t%d\t%d \n", OSTimeGet(), OS_LOWEST_PRIO, OSPrioHighRdy);
...
static  void  OS_InitTaskIdle (void)
{
    ...
    (void)OSTaskCreateExt(OS_TaskIdle,
                          ...
                          OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR,0,0);/* Enable stack checking + clear stack  */
    #else
    (void)OSTaskCreateExt(OS_TaskIdle,
                          (void *)0,                                 /* No arguments passed to OS_TaskIdle() */
...
static  void  OS_InitTaskStat (void)
{
    ...
    (void)OSTaskCreateExt(OS_TaskStat,
                          ...
                          (void *)0,                                   /* No TCB extension               */
                          OS_TASK_OPT_STK_CHK | OS_TASK_OPT_STK_CLR,0,0);  /* Enable stack checking + clear  */
    #else
    (void)OSTaskCreateExt(OS_TaskStat,
                          (void *)0,                                   /* No args passed to OS_TaskStat()*/
